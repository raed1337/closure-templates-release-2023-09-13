...
public final class GeneratePySanitizeEscapingDirectiveCode
  ...
  protected String escapeOutputString(String input) {
    return escapeString(input, "\\'\"\b\f\n\r\t");
  }

  private String escapeString(String input, String escapeCharacters) {
  ...
  protected String convertFromJavaRegex(Pattern javaPattern) {
    String body = convertPatternBody(javaPattern);
    if ((javaPattern.flags() & Pattern.DOTALL) != 0) {
      throw new IllegalArgumentException("Pattern " + javaPattern + " uses DOTALL.");
    }

    return buildRegexPattern(javaPattern, body);
  }

  private String convertPatternBody(Pattern javaPattern) {
    return javaPattern
        .pattern()
        .replace("\r", "\\r")
        .replace("\n", "\\n")
  ...
  private String buildRegexPattern(Pattern javaPattern, String body) {
  ...
  protected void generateHelperFunction(StringBuilder outputCode, DirectiveDigest digest) {
    ...
    addFilterCheck(outputCode, digest);
    handleEscapes(outputCode, digest);
    outputCode.append("\n");
  }

  private void addFilterCheck(StringBuilder outputCode, DirectiveDigest digest) {
  ...
  private void handleEscapes(StringBuilder outputCode, DirectiveDigest digest) {
