...
public abstract class BasicEscapeDirective
  ...
  public SoyExpression applyForJbcSrc(JbcSrcPluginContext context, SoyExpression value, List<SoyExpression> args) {
    return value.isBoxed() 
        ? SoyExpression.forString(javaSanitizer(SoyValue.class).invoke(value)) 
        : SoyExpression.forString(javaSanitizer(String.class).invoke(value.coerceToString()));
  }
  ...
  public final AppendableAndOptions applyForJbcSrcStreaming(JbcSrcPluginContext context, Expression delegateAppendable, List<SoyExpression> args) {
    ...
    if (sanitizerMethod == null) {
      sanitizerMethod = MethodRef.create(Sanitizers.class, name.substring(1) + "Streaming", LoggingAdvisingAppendable.class);
      javaStreamingSanitizer = sanitizerMethod;
    }
    Expression streamingSanitizersExpr = sanitizerMethod.invoke(delegateAppendable);
    return isCloseable() 
        ? AppendableAndOptions.createCloseable(streamingSanitizersExpr) 
        : AppendableAndOptions.create(streamingSanitizersExpr);
  }
    ...
    public SoyExpression applyForJbcSrc(JbcSrcPluginContext context, SoyExpression value, List<SoyExpression> args) {
      if (!value.isBoxed()) {
        ...
        if (value.soyRuntimeType().isKnownNumber()) {
          return SoyExpression.forString(javaSanitizer(double.class).invoke(value.coerceToDouble()));
        }
        if (value.soyRuntimeType().isKnownBool()) {
          return SoyExpression.forString(javaSanitizer(boolean.class).invoke(value.coerceToBoolean()));
        }
        value = value.box();
      }
  ...
  @SoyPurePrintDirective
  static final class FilterTrustedResourceUri extends BasicEscapeDirective implements ShortCircuitable {
  ...
  @SoyPurePrintDirective
  public static final class WhitespaceHtmlAttributesDirective extends BasicEscapeDirective implements Streamable {
